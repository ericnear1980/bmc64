...
Set in `CNetInitTask::Run()` in `kernel.cpp`. `CNetSubSystem::Initialize()` starts `CDHCPClient` internally and polls for a lease for up to 30 seconds. If no lease is granted, networking is silently skipped. TFTP server starts automatically after net init.

**TFTP usage** (from host, SD card must be inserted in Pi; replace IP with the one shown in Network Info):
```bash
...
Avoid em dashes. Use parenthetical phrases or split sentences instead.
