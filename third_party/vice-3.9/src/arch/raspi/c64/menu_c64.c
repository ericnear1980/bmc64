...
static void menu_value_changed(struct menu_item *item) {
   switch (item->id) {
      ...
      case MENU_REU_SIZE:
         ...
         break;
      case MENU_TFE_ENABLED:
         if (item->value) {
            resources_set_int("ETHERNETCARTMode", 0); /* TFE mode */
            resources_set_int("ETHERNETCARTBase", 0xde00);
            resources_set_int("ETHERNETCART_ACTIVE", 1);
         } else {
            resources_set_int("ETHERNETCART_ACTIVE", 0);
         }
         break;
      case MENU_SWIFTLINK_ENABLED:
         resources_set_int("Acia1Enable", item->value);
         break;
      default:
         break;
   }
...
struct menu_item* emux_add_cartridge_options(struct menu_item* root) {
  ...
  int tfe_active = 0;
  resources_get_int("ETHERNETCART_ACTIVE", &tfe_active);
  struct menu_item* tfe_item =
      ui_menu_add_toggle(MENU_TFE_ENABLED, parent, "TFE Ethernet (CS8900A)", tfe_active);
  tfe_item->on_value_changed = menu_value_changed;

  int swiftlink_active = 0;
  resources_get_int("Acia1Enable", &swiftlink_active);
  struct menu_item* swift_item =
      ui_menu_add_toggle(MENU_SWIFTLINK_ENABLED, parent, "SwiftLink/ACIA ($DE00)", swiftlink_active);
  swift_item->on_value_changed = menu_value_changed;
