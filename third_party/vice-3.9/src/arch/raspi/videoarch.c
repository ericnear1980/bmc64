  ...
  for (int i = 0; i < p->num_entries; i++) {
    circle_set_palette_fbl(layer, i,
                   COLOR16(p->entries[i].red, p->entries[i].green,
                           p->entries[i].blue));
    // Tell VICE's render engine to use palette index i for color i (8bpp indexed)
    video_render_setphysicalcolor(canvas->videoconfig, i, i, 8);
  }
...
static uint8_t *circle_fbl[2] = { NULL, NULL };
static int circle_fbl_w[2] = { 0, 0 };   /* draw_buffer_width */
static int circle_fbl_h[2] = { 0, 0 };   /* draw_buffer_height */
static int circle_fbl_pitch[2] = { 0, 0 }; /* Circle buffer pitch (aligned) */

static void alloc_circle_fbl_for_canvas(struct video_canvas_s *canvas,
                                        int canvas_index, int layer,
                                        unsigned int w, unsigned int h) {
  int pitch = 0;
  check_dimensions(canvas, canvas_index, w, h,
                   canvas_index == VDC_INDEX ? raster2_lines : raster_lines);
  circle_alloc_fbl(layer, 0 /* indexed */, &circle_fbl[canvas_index],
                   w, h, &pitch);
  if (circle_fbl[canvas_index]) {
    circle_fbl_w[canvas_index] = w;
    circle_fbl_h[canvas_index] = h;
    circle_fbl_pitch[canvas_index] = pitch;
    emux_frame_buffer_changed(layer);
  }
}
...
static struct video_canvas_s *video_canvas_create_vic(
  ...
  *height = *height * canvas_state[VIC_INDEX].raster_skip;
  ...
  // Circle FBL allocation is deferred to video_canvas_refresh where
  // draw_buffer dimensions are guaranteed to be valid.
...
static struct video_canvas_s *video_canvas_create_vdc(
  ...
  *height = *height * canvas_state[VDC_INDEX].raster_skip;
  ...
  alloc_circle_fbl_for_canvas(canvas, VDC_INDEX, FB_LAYER_VDC, *width, *height);
...
// Circle hardware framebuffer per canvas, allocated once on first refresh.
void video_canvas_refresh(struct video_canvas_s *canvas, unsigned int xs,
                          unsigned int ys, unsigned int xi, unsigned int yi,
                          unsigned int w, unsigned int h) {
  int canvas_index = is_vdc(canvas) ? VDC_INDEX : VIC_INDEX;
  int layer = is_vdc(canvas) ? FB_LAYER_VDC : FB_LAYER_VIC;

  // In VICE 3.9, VICE renders into its own draw_buffer (allocated by raster.c).
  // We copy the full draw_buffer into the Circle hardware framebuffer each frame.
  // The Circle FBL was allocated in video_canvas_create with draw_buffer dimensions.
  // We never re-allocate here — that would hit the assert(!allocated_) in fbl.Allocate.
  // Allocate Circle FBL on first call with valid draw_buffer dimensions.
  if (circle_fbl[canvas_index] == NULL && canvas->draw_buffer) {
    unsigned int fb_w = canvas->draw_buffer->draw_buffer_width;
    unsigned int fb_h = canvas->draw_buffer->draw_buffer_height;
    if (fb_w > 0 && fb_h > 0) {
      alloc_circle_fbl_for_canvas(canvas, canvas_index, layer, fb_w, fb_h);
      /* Don't call circle_show_fbl here — it's called by emux_ensure_video()
       * from vsyncarch_postsync once the emulator is actually running. */
    }
  }

  // Ensure VICE's color/render tables are initialised before we copy pixels.
  if (canvas->draw_buffer && !canvas->videoconfig->color_tables.updated) {
    video_color_update_palette(canvas);
  }

  if (circle_fbl[canvas_index] && canvas->draw_buffer &&
      canvas->draw_buffer->draw_buffer) {
    unsigned int src_w = canvas->draw_buffer->draw_buffer_width;
    unsigned int src_h = canvas->draw_buffer->draw_buffer_height;
    unsigned int src_pitch = canvas->draw_buffer->draw_buffer_pitch;
    unsigned int dst_pitch = (unsigned int)circle_fbl_pitch[canvas_index];
    if (dst_pitch == 0) dst_pitch = src_w;
    uint8_t *src = canvas->draw_buffer->draw_buffer;
    uint8_t *dst = circle_fbl[canvas_index];
    unsigned int copy_w = src_w < (unsigned int)circle_fbl_w[canvas_index] ?
                          src_w : (unsigned int)circle_fbl_w[canvas_index];
    unsigned int copy_h = src_h < (unsigned int)circle_fbl_h[canvas_index] ?
                          src_h : (unsigned int)circle_fbl_h[canvas_index];
    for (unsigned int row = 0; row < copy_h; row++) {
      memcpy(dst + row * dst_pitch, src + row * src_pitch, copy_w);
    }
  }
  ...
  while (pending_emu_joy.head != pending_emu_joy.tail) {
    ...
    if (vkbd_enabled) {
      ...
      case PENDING_EMU_JOY_TYPE_ABSOLUTE:
        ...
        joyport_io_sim_set_poty((pending_emu_joy.value[i] & POTY_BIT_MASK) >> 13,
                                pending_emu_joy.port[i]-1);
        break;
